...
async function checkedPath(value, fs, expected, canonicalized = false) {
  ...
  if (canonicalKey(path.resolve(real)) !== canonicalKey(resolved)) {
    if (canonicalized) fail("SOURCE_CHANGED");
    // Case-insensitive filesystems and Windows short names can change spelling
    // without links. Recheck the canonical path and identity, not the OS.
