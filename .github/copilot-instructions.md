...
- Native-session usage can be imported only through the user's explicit [local collection flow](../docs/local-usage.md). Do not scan private session stores, bind the newest session/run automatically, or combine imported totals with overlapping SDK/manual measurements. Text-derived estimates and source-reported Copilot credit-equivalent usage are separately labelled; never submit either as measured tokens or provider/API `estimatedCost`.
