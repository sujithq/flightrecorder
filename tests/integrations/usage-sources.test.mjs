...
function errorCode(code, ErrorType = SourceError) {
  return error => {
...
async function sourcesForPlatform(platform) {
  const url = new URL("../../extensions/flight-recorder/usage-sources.cjs", import.meta.url);
  const module = { exports: {} };
  runInNewContext(await fs.readFile(url, "utf8"), {
    module, require: createRequire(url), process: { platform }, Buffer
  }, { filename: fileURLToPath(url) });
  return module.exports;
}

async function nonWindowsLstat(file) {
  const stat = await fs.lstat(file);
  if (process.platform !== "win32" || !stat.isFile()) return stat;
  // Model consistent non-Windows metadata despite Windows lstat/handle ctime lag.
  const handle = await fs.open(file);
  try { return await handle.stat(); }
  finally { await handle.close(); }
}

async function aliasedSource() {
  const source = await fixture(chat(request({ promptTokens: 2, outputTokens: 0 })));
  const canonical = await fs.realpath(source.path);
  const alias = path.join(path.dirname(canonical), path.basename(canonical).toUpperCase());
  const target = file => file === alias ? canonical : file;
  return {
    source: { ...source, path: alias }, canonical,
    fs: {
      ...fs,
      lstat: file => nonWindowsLstat(target(file)),
      realpath: file => fs.realpath(target(file))
    }
  };
}
...
test("canonical path aliases preserve source identity on non-Windows filesystems", async t => {
  for (const platform of ["darwin", "linux"]) await t.test(platform, async () => {
    const reader = await sourcesForPlatform(platform);
    const alias = await aliasedSource();
    const snapshot = await reader.readSource(alias.source, { fs: alias.fs });
    const canonical = await reader.readSource({ ...alias.source, path: alias.canonical }, { fs: alias.fs });
    assert.deepEqual(snapshot, canonical);
    assert.equal(snapshot.observations[0].inputTokens, 2);
    assertPrivate(snapshot);
  });
});

test("canonical path aliases still reject identity changes, links and unstable resolution", async t => {
  const reader = await sourcesForPlatform("darwin");
  const alias = await aliasedSource();
  ...
  await t.test("canonical path became a symlink", async () => {
    const linked = { ...alias.fs, lstat: async file => {
      const stat = await alias.fs.lstat(file);
      if (file === alias.canonical) stat.isSymbolicLink = () => true;
      return stat;
    } };
    await assert.rejects(reader.readSource(alias.source, { fs: linked }), errorCode("SOURCE_UNAVAILABLE", reader.SourceError));
  });
  await t.test("canonical resolution changes again", async () => {
    let calls = 0;
    const unstable = { ...alias.fs, realpath: async file => {
      calls++;
      return file === alias.canonical ? alias.source.path : alias.fs.realpath(file);
    } };
    await assert.rejects(reader.readSource(alias.source, { fs: unstable }), errorCode("SOURCE_CHANGED", reader.SourceError));
    assert.equal(calls, 2);
  });
});

test("case-sensitive macOS paths remain distinct sources", async () => {
  const reader = await sourcesForPlatform("darwin");
  const alias = await aliasedSource();
  const separate = await fixture(chat(request({ promptTokens: 5 })));
  const caseSensitive = {
    ...fs,
    lstat: file => nonWindowsLstat(file === alias.source.path ? separate.path : file),
    realpath: file => file === alias.source.path ? Promise.resolve(file) : fs.realpath(file),
    open: (file, flags) => fs.open(file === alias.source.path ? separate.path : file, flags)
  };
  const upper = await reader.readSource(alias.source, { fs: caseSensitive });
  const lower = await reader.readSource({ ...alias.source, path: alias.canonical }, { fs: caseSensitive });
  assert.notEqual(upper.sourceId, lower.sourceId);
  assert.equal(upper.observations[0].inputTokens, 5);
  assert.equal(lower.observations[0].inputTokens, 2);
});
...
test("discovery does no implicit probing and stays within supplied known layouts", async () => {
  ...
  // Model macOS case-insensitive realpath behavior even on Windows/Linux CI.
  const canonicalPaths = new Map();
  for (const file of paths) {
    for (let current = file; current !== path.dirname(current); current = path.dirname(current)) {
      canonicalPaths.set(current.toLowerCase(), current);
    }
  }
  const target = file => canonicalPaths.get(file.toLowerCase()) ?? file;
  const opened = new Set();
  const caseInsensitive = {
    ...fs,
    lstat: file => fs.lstat(target(file)),
    realpath: file => fs.realpath(target(file)),
    opendir: async file => {
      assert.ok(!opened.has(file), "Canonical directories must only be enumerated once.");
      opened.add(file);
      return fs.opendir(file);
    }
  };
  const reader = await sourcesForPlatform("darwin");
  const macDescriptors = await reader.discoverSources({ vscodeUserPath: user, copilotHome: home, fs: caseInsensitive });
  assert.deepEqual(new Set(macDescriptors.map(item => item.path)), new Set(paths));
  assert.equal(macDescriptors.length, paths.length);
});
